...
public class SplitArrayIntoConsecutiveSubsequences659 {
    ...
    /**
     * solution I: Hash table + minimum heap
     * @param nums
     * @return
     */
    public boolean isPossible(int[] nums) {
        ...
        for (int x : nums) {
            ...
            if (map.containsKey(x - 1)) {
                Integer prevLength = map.get(x - 1).poll();
    ...
    public boolean isPossibleII(int[] nums) {
        ...
        for (int x : nums) {
            ...
            if (count > 0) {
                Integer prevEndCount = endMap.getOrDefault(x - 1, 0);
                if (prevEndCount > 0) {
                    countMap.put(x, count - 1);
                    endMap.put(x - 1, prevEndCount - 1);
                    endMap.put(x, endMap.getOrDefault(x, 0) + 1);
                } else {
                    Integer count1 = countMap.getOrDefault(x + 1, 0);
                    Integer count2 = countMap.getOrDefault(x + 2, 0);
